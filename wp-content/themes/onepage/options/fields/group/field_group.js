jQuery(document).ready(function($){
    ...
    $("div[id=nhp-opts-groups-accordion]")
        ...
        $('.nhp-opts-groups-add').click(function () {
            ...
            // Count # of groups
            var groupCount = parseInt(groupCounter.val()) + 1;
            // Update the groupCounter
            groupCounter.val(groupCount);
            ...
            $(newGroup).find('label').each(function() {
                var $this = $(this);
                $this.attr("for", $this.attr("for").replace("@", groupCount));

            });

            $(newGroup).find('input[type="text"], input[type="radio"], input[type="number"], input[type="hidden"], textarea , select').each(function(){
                var $this = $(this);
                ...
                if($this.prop("tagName") == 'SELECT') {
                    ...
                    $(newGroup).find('input[type=radio][value="'+std_val+'"]').prop('checked', true);
                } else {
					$this.val(std_val);
				}
